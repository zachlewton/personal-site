...
async function handleLocalBackend(originalConfig) {
  ...
  let mergedConfig = (0, _deepmerge.default)(originalConfig, {
    backend: {
      name: 'proxy',
      proxy_url: proxyUrl
    }
  ...
  if (mergedConfig.publish_mode && publish_modes && !publish_modes.includes(mergedConfig.publish_mode)) {
    const newPublishMode = publish_modes[0];
    mergedConfig = (0, _deepmerge.default)(mergedConfig, {
      publish_mode: newPublishMode
    });
    console.log(`'${mergedConfig.publish_mode}' is not supported by '${type}' backend, switching to '${newPublishMode}'`);
  }
...
function loadConfig(manualConfig = {}, onLoad) {
  ...
  return async dispatch => {
    ...
    try {
      ...
      let mergedConfig = (0, _deepmerge.default)(configYaml, manualConfig);
      (0, _configSchema.validateConfig)(mergedConfig);
      mergedConfig = await handleLocalBackend(mergedConfig);
      const config = applyDefaults(normalizeConfig((0, _immutable.fromJS)(mergedConfig)));
